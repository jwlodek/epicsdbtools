...
def test_tokenizer_mixed_line(tokenizer_factory):
    tokens = list(
        tokenizer_factory('bareword "$(NAME=VALUE)" name=value "" {name} # comments')
    )
    assert len(tokens) == 9
...
def test_tokenizer_with_epics_record_signature(
    tokenizer_factory, trailing_open_bracket
):
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            'field(INP, "@asyn($(PORT),0,1)TST_BI_ASYNINT32")',
            ["field", "(", "INP", ",", "@asyn($(PORT),0,1)TST_BI_ASYNINT32", ")"],
        ),
        (
            'field(OUT, "@asyn($(PORT),0,1)TST_BO_ASYNINT32")',
            ["field", "(", "OUT", ",", "@asyn($(PORT),0,1)TST_BO_ASYNINT32", ")"],
        ),
        (
            'field(FLNK, "XF:31ID1-ES{S:TEST}TEST")',
            ["field", "(", "FLNK", ",", "XF:31ID1-ES{S:TEST}TEST", ")"],
        ),
    ],
)
